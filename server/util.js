...
export function randomizeRoles(game, round, numSLPairs, reqMutual) { 
  ...
  while (speakerListenerPairs.length < numSLPairs) {
    ...
    if (playerPool.includes(speakerId) && playerPool.includes(listenerId)){
      ...
      let pair = {speaker: speakerId, listener: listenerId, availableAdvisors: [], chosenAdvisors: []}
  ...
  // In a passive game, if a player leaves, there will be one player without a pair;
  // Make that player a listener and provide them with a speaker.
  if (playerPool.length > 0 && game.treatment.passiveMode) {
  ...
  // In an active game, leftover players would become advisors
  ...
  // Update all roles with SL pairs
  round.set("allRoles", speakerListenerPairs);
...
export function assignRequestsToAdvisors(game, round) {
  ...
  for (let team of allRolesPerRound) {
    ...
    // Randomly assign advisors to a listener's chosen advisors
    // TODO: Probably find new algorithm that ensures that all advisors are used.
    while (chosenAdvisors.length < numAdvisorsPerPair) {
      var randomAdvisor = availableAdvisors[_.random(availableAdvisors.length-1)];
      var usedAdvisor = _.remove(availableAdvisors, (s) => s === randomAdvisor);
    ...
    for (let advisor of chosenAdvisors) {
      // Set the request queue for the advisor
      const advisorPlayer = game.players.find((p) => p.get("nodeId") === advisor);
      const requestQueue = advisorPlayer.round.get("requestQueue");

      // Send the puzzle from the Listener's view
  ...
  // Update all roles with chosen advisor changes
  round.set("allRoles", allRolesPerRound);


  // Shuffle the advisor's request queue
  game.players.forEach((player) => {
